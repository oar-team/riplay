...
#define TOO_MUCH_TIME 8.186        /* no more than one hour (90% jobs in curie) for runtime  */
#define TOO_MUCH_ARRIVE_TIME 8.186 /* no more than one hour for arrivetime */
...
/* In curie, no partition is dedicated to interactive jobs, it depends of the submission type. 
As we don't know yet we put 0: all batch jobs. Thus we remove the model parameters for bacth
and interactive and keep only the "whole sample" parameters */
 ...
 */
/* length(which(swf_curie_clean$proc_alloc==1))/length(swf_curie_clean$job_id) 
gives 0.07956204 if we take curie_CLEAN.swf and remove zero sized jobs
*/
#define SERIAL_PROB       0.07956
#define POW2_PROB         0.5
#define ULOW              1       /* The smallest parallel job is 2 cores */
#define UMED              10        
#define UHI               16.29921         /* SYSTEM SIZE is 2^UHI == 128          */
#define UPROB             0.8    
...
inline void arrive_init(double *aarr , double*barr , double *anum, double *bnum,
...
inline double gamrnd(double alpha , double beta);
inline double gamrnd_int_alpha(unsigned n , double beta);
inline double gamrnd_alpha_smaller_1(double alpha, double beta);
inline double betarnd_less_1(double alpha , double beta);
inline double gamcdf(double x , double alpha , double beta);
inline double gser(double x , double a);
inline double gcf(double x , double a);
inline double two_stage_uniform(double low, double med, double hi, double prob);
